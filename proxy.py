...
def receive_from(connection):
	buffer = b""
	connection.settimeout(5) # increase if having issues with timeouts due to distace or poor networks
	try:
		while True:
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
	...
	while True:
		...
		if len(remote_buffer):
			print("[<--] Received %d btyes from remote" % len(remote_buffer))
			hexdump(remote_buffer)
			remote_buffer = response_handler(remote_buffer)
			client_socket.send(remote_buffer)
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
	...
	try:
		...
		print("!!! There was a problem: %r" % e)
		print("!!! Failed to listen on %s:%d" % (local_host, local_port))
		print("!!! Check for other listening sockets or correct perissions.")
		sys.exit()
